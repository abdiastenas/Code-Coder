...
export class Input {
    ...
    static async getForm (
        message: string,
        choices: Choices ): Promise<UserFormInput> {
    ...
    static async getSelect (
        message: string,
        choices: SelectChoices ) : Promise<UserOption> {
        const input: UserOptionAux = await prompt ({
            type: 'select',
            name: 'data',
            message: message,
            choices: choices.map((choices: SelectChoice) => ({
                name: choices.option.toString(),
    ...
    static async getSelectById (
        message: string,
        choices: Choice[] ) : Promise<UserOptionById> {
